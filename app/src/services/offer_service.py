...
async def change_offers(offers_data: list[OfferChange]):
    async with async_session() as session:
        ...
        changed_offers = utils.update_offers_data(offers_df, changes)
...
async def setup_offers_data():
    ...
    data = utils.build_offers_data(yandex_offers, setup_mode=True)
...
async def update_offers():
    ...
    offers_df.sort_values(by='sku', inplace=True)
    yandex_offers_df.sort_values(by='sku', inplace=True)

    print(offers_df['sku'])
    print(yandex_offers_df['sku'])
...
async def build_csv():
    ...
    df = df[OfferOut.model_fields.keys()]
